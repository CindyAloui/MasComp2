...
def update(buffer, nouns):
    ...
    for line in buffer.split("\n"):
        ...
        if line[6] == "1":
            for i in range(len(word_positions)):
                ...
                if len(new_positions_left) >= position_size or len(new_positions_right) >= position_size or occ[current_nouns[i]] > maxOcc:
                    continue
    ...
    if words_left:
        ...
        for i in range(len(predictions)):
            ...
            w = n[i]
            occ[w] += 1
            num[w] += prediction * pow(2 * (0.5 - prediction), exponent)
    ...
    #     current_morphos.append(get_index(line[2], vocab_morphos))
    #     current_positions.append(p)
    #     if line[1] == "NOUN" and line[3] in nouns and occ[line[3]] < maxOcc:
    #         word_positions.append(p)
    #         current_nouns.append(line[3])
    #     if line[6] == "1":
    #         for i in range(len(word_positions)):
